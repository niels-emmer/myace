...
> store holding **17 starter agent profiles** (3 base + 14 specializations).
...
- **Starter packs out of the box** — every fresh install seeds itself with 3
  base collections (Vibecoder, Software Engineer, Data Scientist) and 14
  specializations (Frontend, Backend, Infrastructure as Code Expert, Azure
  Cloud Architect, AWS Cloud Architect, Security Auditor, Documentation
  Editor, Full-Stack Developer, DevOps/Platform Engineer, Java/Spring
  Developer, iOS Developer, Android Developer, Spec-Driven Development,
  AI/LLM Engineering), so there's real, opinionated content to build a first
  profile from on day one.
