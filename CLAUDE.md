...
**Deployment hardening (`app/main.py`, `app/core/config.py`).** The `APP_SECRET_KEY`-still-default startup warning has two siblings now: `DEBUG=true` and `ADMIN_BOOTSTRAP_ENABLED=true` also log a warning whenever `app_env != "development"`, since both are safe dev defaults that are unsafe left on for a public deployment. `TrustedHostMiddleware` is registered only if `settings.trusted_host_list` is non-empty (`TRUSTED_HOSTS` env, comma-separated) — a no-op until an operator sets their real domain, since a forked/self-hosted app can't hardcode one. `backend/Dockerfile`'s uvicorn `CMD` runs with `--proxy-headers --forwarded-allow-ips=*` (safe specifically because `docker-compose.prod.yml` exposes no host port — only the reverse-proxy container on the same Docker network can reach it) so `X-Forwarded-Proto` is trusted and OIDC `redirect_uri` generation (`request.url_for(...)` in `auth.py`) resolves to `https://` behind a reverse proxy; `--reload` moved out of the Dockerfile into `docker-compose.dev.yml`'s `command:` override, since the base compose file is meant to run unattended. `Settings.api_key_salt` was removed entirely (was declared but never referenced anywhere — `security.py`'s `hash_api_key` uses `bcrypt.gensalt()` per-hash instead) rather than wiring it into a config value that did nothing.
