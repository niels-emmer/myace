...
```
Adapters are stateless. The backend registers seven (`backend/app/adapters/__init__.py`): `claude_code` (→ `CLAUDE.md`, `.claude/agents/*.md`, `.claude/workflows/*.md`), `opencode` (→ `.opencode/skills/*.json`, `.opencode/agents/*.json`, `AGENTS.md`), `cursor` (→ `.cursorrules`, `.cursor/rules/*.mdc`, `.cursor/workflows/*.mdc`), plus `codex_cli`, `copilot_cli`, `cline`, and `windsurf`. `ProfileCompileRequest.target` (`app/models/profile.py`) is a `Literal` of the seven adapters' primary `adapter_name()` — keep it in sync whenever an adapter is added, since a name missing from that Literal 422s before ever reaching `compile_profile()`/`get_adapter()`, even though the adapter itself works fine. Each adapter's `supported_targets()` additionally lists a few alias strings (e.g. `claude-code`/`claude`) purely as adapter metadata surfaced by `GET /api/v1/adapters` — only the primary `adapter_name()` is ever a valid lookup key or compile `target`, so UI code building a target picker should read `adapter.name`, not flatten `adapter.targets`. The CLI keeps its own adapter copies as a fallback for offline/no-server use, currently only for the original three (`cli/myace_cli/adapters/`) — `codex_cli`/`copilot_cli`/`cline`/`windsurf` are backend/web-UI-only for now.
...
### Starter packs (`app/services/seed_collections.py`)
`collections/base/<slug>/` and `collections/additional/<slug>/` hold hand-authored example content — 2 base packs (`vibecoder`, `software-engineer`) and 5 additional/goal-specific packs (`frontend`, `backend`, `iac-expert`, `auditor`, `editor`) — written in the same on-disk shape the scanner reads (`skills/<name>/SKILL.md`, `agents/<name>.md`, `commands/<name>.md`, `AGENTS.md`; no `opencode.json` — model configs are host-specific and out of scope for shared content). `seed_starter_collections()` runs from `app/main.py`'s `lifespan` on every backend startup and turns each of these directories into a `Collection` + `Artifact` row set, owned by a dedicated, **passwordless** system account (`starter-packs@myace.local` — `password_hash` stays `None`, so it can never authenticate via `/auth/login`, which requires a hash to verify against; it exists purely to satisfy `Collection.owner_id`'s `NOT NULL` FK). Every seeded collection ships `published=True` + `visibility="public"` + `is_starter_pack=True`, so it appears in Community Collections immediately with no separate publish step. Idempotent: each collection is looked up by `name` + `is_starter_pack` before insertion, so repeated boots/restarts/multi-replica startups never duplicate it — safe to call unconditionally (unlike `init_db()`, which is dev-only). Seeding failures (e.g. the DB schema not existing yet because an operator hasn't run `alembic upgrade head` on a brand-new deployment) are caught and logged rather than raised, so a seeding problem never blocks the app from starting; the next restart after migrations land picks it back up. `docker-compose.yml` bind-mounts the repo-root `./collections:/app/collections:ro` into the backend container (present in both dev and prod via the base compose file); `Settings.collections_root` (`app/core/config.py`) points at that path by default — override it if running the backend outside Docker. `seed_collections.py` deliberately does **not** call the public `scanner.scan_directory()` — that function's `_resolve_path()` confines scans to `settings.scan_root` (`/host-home`) as a security boundary against arbitrary *user-supplied* local-machine paths, which doesn't apply to our own hardcoded, trusted `STARTER_COLLECTIONS` paths. Instead it reuses the scanner's private per-file parsers (`_parse_skill_file`, `_parse_agent_file`, `_parse_command_file`, `_parse_agents_md`) directly against a local directory walk, so the on-disk format stays perfectly in sync with what the scanner (and therefore the community-import flow) already understands. To add a new starter pack: create the directory under `collections/{base,additional}/<slug>/` in the scanner's format, then add an entry to `STARTER_COLLECTIONS` in `seed_collections.py` with its display `name`/`category`/`description` — the matching `Collection`/`Artifact` rows appear on the next backend restart, no migration needed unless you're changing the `Collection`/`Artifact` schema itself.
...
**OIDC login uses PKCE (S256).** The `/auth/login/{provider}` route generates a `code_verifier` (stored in the session) and sends a `code_challenge` with the authorize redirect. The callback does not need to verify the challenge explicitly — Authlib handles that server-side. Never skip PKCE when adding a new OIDC provider.

**Registration never authenticates without a password.** If the email already exists, `/auth/register` returns a fake `UserRead` (random UUID) instead of setting the session or returning the real user — prevents both user enumeration and authentication bypass.
...
First-ever registered user becomes admin automatically, gated on `settings.admin_bootstrap_enabled` (default `true`) — operators are expected to flip it to `false` in `.env` right after creating their own admin account on a public deployment, since it otherwise stays open to *whoever registers next*, not just the first person ever (see ADR-0005's update note). `ADMIN_EMAILS` (config, comma-separated) promotes specific emails on register/OIDC-login regardless of that flag. There is no more "unauthenticated placeholder user" concept — `app/services/placeholder_user.py` (which used to resolve the nil UUID to one shared `local@myace.local` account) was removed once real auth landed; every create route derives ownership from `current_user.id`.
...
**Deployment hardening (`app/main.py`, `app/core/config.py`).** The `APP_SECRET_KEY`-still-default check is now a **`RuntimeError`** in production (was a warning in earlier versions) — the app refuses to start if the session-signing key is the placeholder. `DEBUG=true` and `ADMIN_BOOTSTRAP_ENABLED=true` still log warnings when `app_env != "development"`. `TrustedHostMiddleware` is **always** registered — in development it defaults to `['*']` (allow all), but in production it raises a `RuntimeError` at startup if `TRUSTED_HOSTS` is unset, preventing Host-header injection attacks. `backend/Dockerfile`'s uvicorn `CMD` runs with `--proxy-headers --forwarded-allow-ips=*` (safe specifically because `docker-compose.prod.yml` exposes no host port — only the reverse-proxy container on the same Docker network can reach it) so `X-Forwarded-Proto` is trusted and OIDC `redirect_uri` generation (`request.url_for(...)` in `auth.py`) resolves to `https://` behind a reverse proxy; `--reload` moved out of the Dockerfile into `docker-compose.dev.yml`'s `command:` override, since the base compose file is meant to run unattended. `Settings.api_key_salt` was removed entirely (was declared but never referenced anywhere — `security.py`'s `hash_api_key` uses `bcrypt.gensalt()` per-hash instead) rather than wiring it into a config value that did nothing.
...
### Frontend
React Router SPA (`src/App.tsx`) with pages under `src/pages/` (Login, Dashboard, CollectionsManager, CollectionDetail, CommunityCollections, CommunityCollectionDetail, ImportPage, ProfileComposer, `ProfileDetail` mounted at `/profiles/:id`, `TargetExporter` mounted at `/compile`, UserSettings, SystemSettings) and a single `Layout` shell. `SystemSettings` sits behind a `RequireAdmin` route wrapper (`App.tsx`) that redirects non-admins to `/` — everything else just needs `RequireAuth`. (`/export` 301-redirects to `/compile` for old links; the component file is still named `TargetExporter.tsx`.) `src/lib/api.ts` is the API client (every call sets `credentials: 'same-origin'` so the session cookie flows — any hand-rolled `fetch()` outside `api.ts` must set this too, see `ImportPage.tsx`'s scan/import calls); `src/types/index.ts` mirrors the backend's Canonical IR shape. `src/contexts/ThemeContext.tsx` handles light/dark/system theme; `src/contexts/AuthContext.tsx` (same provider/hook shape) holds `user`/`isLoading` and the `login`/`register`/`logout`/`loginWithProvider` actions, calling `GET /auth/me` on mount to rehydrate an existing session.
...
**`UserSettings.tsx`**'s "CLI Setup" block is generated, not static copy — `installCommand`/`loginCommand`/`pullCommand` interpolate `window.location.origin` and the just-created token (`newToken`) the same way `TargetExporter.tsx` builds its `myace pull` string. Don't hardcode a server URL or placeholder token back into this block; a wrong one (e.g. `pip install myace-cli`, which was never published to PyPI) silently breaks first-run onboarding.
...
- **`main` is protected** — CI must pass, at least one approving review required, stale reviews dismissed on new pushes. Never push directly to `main`.
- Never log tokens, secrets, or PII.
- **Soft-delete everything.** Artifacts, profiles, doc cache entries use `deleted_at`. Collections and API tokens use `is_active = False`. Never call `session.delete()` on user data.
- **Use `Literal` types for constrained string fields** (`artifact_type`, `source_type`, `target`) — catches invalid values at the schema validation layer (422) instead of business logic (400).
