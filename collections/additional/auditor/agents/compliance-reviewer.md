...
- Confirm new attack surface or data flows carry required documentation (threat-model note, data-classification note). If security-compliance-auditor confirmed no new surface, PASS with "not needed" rather than flagging a gap.
- Check sensitive-data handling against the project's declared classification tiers.
- Report findings in PASS/FAIL/N/A format matching `security-audit-checklist`.
...
Report findings to the requester. Route missing-documentation gaps to the owning engineer or `security-compliance-auditor`. Flag unresolvable compliance questions for human decision.
