"""Direct unit tests for app.services.scanner's private per-file parsers.

No scanner-level test file existed before this — these cover only the new
handoff_to frontmatter field (Epic 3.1) added to _parse_agent_file, not a
general scanner test suite (a pre-existing gap, out of scope here).
"""

from pathlib import Path

from app.services.scanner import _parse_agent_file
...
def test_parse_agent_file_handoff_to_empty_list_is_not_none(tmp_path: Path) -> None:
    ...
    [], not get coerced into None ('not declared') — the two are
    deliberately distinct (see the field's docstring in scanner.py)."""
