...
export default function Dashboard() {
  const { data: collections } = useQuery({
    ...
    queryFn: () => collectionsApi.list({ visibility: 'public' }),
  });

  const { data: profiles } = useQuery({
    queryKey: ['profiles'],
    queryFn: () => profilesApi.list({ is_public: true }),
  });
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div className="space-y-3">
          <Step number={1} text="Import a Collection from a Git repository" />
          <Step number={2} text="Create a Profile combining Base + Additional collections" />
          <Step number={3} text="Compile and export to your target framework" />
      ...
      {adapters && adapters.length > 0 && (
        <div>
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Available Adapters</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {adapters.map((adapter) => (
              ...
              >
                ...
                <p className="text-sm text-gray-500 mt-1">{adapter.description}</p>
...
function Step({ number, text }: { number: number; text: string }) {
  return (
    <div className="flex items-center gap-3">
      ...
      {number < 4 && <ArrowRight className="h-4 w-4 text-gray-300 ml-auto" />}
    </div>
  );
}
